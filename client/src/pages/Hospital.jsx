...
out;`;
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} className="bg-red-50">
      <Navbar/>
      <section className="max-w-6xl mx-auto px-6 py-16 flex flex-col md:flex-row items-center justify-between gap-10">
        <div className="max-w-xl">
          <div className="inline-flex items-center gap-2 bg-red-100 px-4 py-2 rounded-full mb-4">
            <ShinyText text="🏥 Find Nearby Hospitals" color="#DC2626"/>
          </div>
          <h1 className="text-5xl font-bold">Nearby <span className="block text-red-600 mt-3 mb-3">Emergency Hospitals</span> Anytime You Need</h1>
          <p className="text-gray-600 mt-6 mb-8">Locate nearby hospitals, emergency care and ambulance services with live navigation.</p>
          <button onClick={()=>{getLocation();mapSectionRef.current?.scrollIntoView({behavior:"smooth"});}} className="bg-red-600 text-white px-6 py-3 rounded-full font-semibold hover:scale-105 transition">Find Nearby Hospitals ➜</button>
        </div>
        <img src={hospitalImg} alt="Hospital" className="hidden md:block w-[300px]"/>
      </section>

      <section ref={mapSectionRef} className="max-w-6xl mx-auto px-6 py-20 relative">
        <h2 className="text-4xl font-bold text-center text-red-600 flex justify-center items-center gap-3 mb-6">
          <HeartPulse size={40}/> Hospitals Near You
        </h2>
        {loading&&<div className="absolute inset-0 flex items-center justify-center bg-white/70"><div className="w-12 h-12 border-4 border-red-600 border-t-transparent rounded-full animate-spin"/></div>}
        <div id="map" className="w-full h-[500px] rounded-xl"/>
      </section>
